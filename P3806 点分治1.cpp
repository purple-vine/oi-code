...
int head[M], cnt1; bool vis[M], ans[M], tf[10000005];
int u, v, w, k, root;
...
queue<int> t;
void deal(int u){
	vis[u] = 1; tf[0] = 1; t.push(0);
	for(int i = head[u]; i; i = e[i].nxt){
		int v = e[i].to; if(vis[v]) continue;
		d[v] = e[i].w; dfs(v, u);
		for(int j = 1; j <= s; j++){
			for(int k = 1; k <= m; k++)
				if(dis[j] <= q[k]) ans[k] |= tf[q[k] - dis[j]];
		}
		for(int j = 1; j <= s; j++)
			if(dis[j] <= 10000000) tf[dis[j]] = 1, t.push(dis[j]);
	}
	while(!t.empty()) tf[t.front()] = 0, t.pop();
	calsiz(u, 0);
	for(int i = head[u]; i; i = e[i].nxt){
		int v = e[i].to; if(vis[v]) continue;
		deal(get(v, siz[v]));
